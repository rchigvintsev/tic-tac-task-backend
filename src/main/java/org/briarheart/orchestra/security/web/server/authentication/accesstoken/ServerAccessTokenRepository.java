...
public interface ServerAccessTokenRepository {
    ...
    /**
     * Saves an access token to the given instance of {@link ServerWebExchange}.
     *
     * @param accessToken access token to be saved (must not be {@code null})
     * @param exchange    exchange to which the given access token should be saved (must not be {@code null})
     * @return completion notification
     */
    Mono<Void> saveAccessToken(AccessToken accessToken, ServerWebExchange exchange);
